...
- step 2 — Verbatim transplant from plan can carry imports that the simplified body never used; warning was silent until rebuild — prevented going forward by: treat any `unused_import` compiler warning as a release blocker
- step 2 — Reaching for a hand-rolled list helper instead of consulting stdlib introduced `let assert` and an unreachable arm; `gleam_stdlib.list.last` already returns `Result(a, Nil)` — prevented going forward by: before defining any `fn` that takes a `List(_)`, check stdlib first
- step 2 — Security invariants that depend on upstream library behavior (e.g. elli not URL-decoding `req.path`) silently rot when the boundary library is swapped — prevented going forward by: every guard that relies on byte-level path equality MUST carry a comment naming the upstream component responsible for not pre-decoding
- step 4 — `string.replace` strips every substring occurrence, not just the suffix — prevented going forward by: `scan_strips_only_md_suffix_test` (fixture `notes.mdx.md` asserts leaf == `"notes.mdx"`) plus convention "use `string.drop_right` for known-suffix stripping"
- step 4 — Positional tuple types hide field intent and force `.0`/`.1`/`.2` accessors in callers — prevented going forward by: convention "named record over positional tuple for any public payload"
- step 4 — Group-survival assertions only via entry-count can mask a filter regression in one rule with another — prevented going forward by: per-rule tests with a sibling valid `.md` so the group survives and the specific filter is the only thing under test
- step 5 — Mixing fail-fast (`let assert`) and silent-fallback (`case ... Error(_) -> []`) in the same handler hides config errors as empty pages — prevented going forward by: treating template read, layout-validate, and scan as a single boot-time invariant block in `main`, never per-request
- step 5 — Re-reading the index template and re-scanning `posts/` on every request is wasted I/O once the data is immutable per process — prevented going forward by: pre-computing `rendered_index` at boot and closing over it in the elli handler
- step 5 — Tests can omit error variants silently (we shipped 3 of 4 `LayoutError` cases) — prevented going forward by: convention that every public ADT variant gets one focused test, file-level so coverage gaps are visible
- step 6 — Code-span content must be opaque to ALL downstream inline passes (links, emphasis), not just emphasis — prevented going forward by: `code_span_suppresses_link_test` pinning `` `[x](y)` `` round-trip; sentinel range extended to `[ ] ( )` inside code content
- step 6 — Attribute-position escaping is its own concern: HTML-entity escape isn't enough; markdown-meaningful chars (`*`, `` ` ``) must be sentineled in href values too — prevented going forward by: paired `link_href_star_protected_test` + `link_text_can_have_emphasis_test` regression set
- step 6 — Sentinel ranges grow as new inline syntax is added; centralize `restore_escapes` so every new sentinel is restored in one place — prevented going forward by: `restore_escapes` consolidates all PUA → literal mappings
- step 6 — `split_last`-based emphasis is greedy across the whole line; document the consequence rather than silently shipping it — prevented going forward by: `two_emphasis_runs_on_one_line_test` with explicit pin comment
- step 6 — Fence openers require a bare `` ``` `` (no language tag); pin the degenerate output so anyone "fixing" it has a baseline diff — prevented going forward by: `fence_with_language_hint_is_paragraph_test`
